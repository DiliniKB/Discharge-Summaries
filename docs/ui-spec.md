...
- **The muted status text under Print/Save explains exactly why they're disabled** — `"Fill in Name, Telephone, BHT to save"`, narrowing as each is fixed, instead of a generic "Not saved" with no visible reason. Needed precisely because nothing is shown red until the user's own blur (above): a disabled button with zero red anywhere on screen would otherwise be a silent dead end (`docs/decisions.md`).
