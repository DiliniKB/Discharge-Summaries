...
**A second bug turned up in the gating itself: Save could be enabled, then clicking it produced a red border AND a "✓ Saved" confirmation at the same time — reported as "save is enabled, but when clicked user [is] alarmed."** Root cause: Save/Print's enabled state reflects Name/Telephone/BHT's *last-blurred* values, but `Editor._commit_focused_field()` (called first thing inside `_on_save`/`_on_print`, to force-commit whatever field the user was still typing in) can itself change validity — if the user was mid-edit in one of those three fields when they clicked, the forced blur it triggers is the *first* validation of that in-progress text. A button that was correctly enabled a moment ago could reveal, at the moment of the click, that what's now on screen isn't actually valid — and the old code flushed and confirmed "✓ Saved" regardless, contradicting the red border that had just appeared right next to it.

Fixed by always calling `controller.flush()` first in both handlers (so whatever's genuinely valid still saves, and the coalesce timer armed by any field's blur is always stopped — bailing out *before* `flush()` would leave a stray-armed timer to fire later against a possibly-closed connection, the same bug class this codebase has been bitten by before), then checking `patient_section.is_valid()` and returning before the confirmation/preview step if it's `False`. The distinction that matters: `flush()` persists whatever's valid unconditionally, matching "per-field blocking, not per-record" above; only the "treat this as done" step (the `✓ Saved` label, or opening Print Preview) is refused when the record as a whole isn't ready.

**A third gap: a disabled Save button with no red shown anywhere is a silent dead end.** Since `patient.py` deliberately never shows red until the user's own blur (the earlier "new card looked already broken" fix), a genuinely untouched brand-new card — or an older record whose Telephone predates this validation — has Save/Print disabled with *nothing* visibly wrong on screen. Reported as "save button still disabled" with no other context, because there was nothing to point to. `PatientSection.missing_required_fields()` names exactly which of Name/Telephone/BHT are currently invalid; `Editor._update_save_print_enabled()` writes it into the existing muted status label (`"Fill in Name, Telephone to save"`, narrowing live as fields are fixed) instead of leaving that label on a generic, unexplained "Not saved."

This has to coexist carefully with the label's other job (reflecting a real `✓ Saved`/`Not saved` state): the "Fill in ..." message is only ever written when actually invalid, and only reverted back to "Not saved" when it was showing "Fill in ..." a moment ago — an ordinary no-op blur on an already-`✓ Saved`, fully-valid record must not make that truthful confirmation flicker back to "Not saved" just because some unrelated field was clicked into and back out of.
